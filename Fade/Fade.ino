...
int stepTime = 9; // 12 to switch off entirely / 9 to make smooth 
int subSteps = 4;
int startDelay = 1000;
...
void zeroCross(){
  ...
  boolean D7_crossed = false;
   ...
   crossTime = micros();
   while (micros() - crossTime < startDelay){   }
  
  for (int i=0;i<255;i++){ 

     if (i>=sine256[D0_pos] && !D0_crossed){
        PORTB |= _BV(5);//13
        D0_crossed = true;
     }
     if (i>=sine256[D1_pos] && !D1_crossed){
        PORTB |= _BV(4);//12
        D1_crossed = true;
     }
     if (i>=sine256[D2_pos] && !D2_crossed){
        PORTB |= _BV(3);//11
        D2_crossed = true;
     }
     if (i>=sine256[D3_pos] && !D3_crossed){
        PORTB |= _BV(2);//10
        D3_crossed = true;
     }
     if (i>=sine256[D4_pos] && !D4_crossed){
        PORTB |= _BV(1);//9
        D4_crossed = true;
     }
     if (i>=sine256[D5_pos] && !D5_crossed){
        PORTB |= _BV(0);//8
        D5_crossed = true;
     }
     if (i>=sine256[D6_pos] && !D6_crossed){
        PORTD |= _BV(7);//7
        D6_crossed = true;
     }
     if (i>=sine256[D7_pos] && !D7_crossed){
        PORTD |= _BV(6);//6
        D7_crossed = true;
     }       
   ...
   if (D0_pos>255) D0_pos-=255;
   if (D1_pos>255) D1_pos-=255;
   if (D2_pos>255) D2_pos-=255;
   if (D3_pos>255) D3_pos-=255;  
   if (D4_pos>255) D4_pos-=255;
   if (D5_pos>255) D5_pos-=255; 
   if (D6_pos>255) D6_pos-=255; 
   if (D7_pos>255) D7_pos-=255; 
